...
describe('the parser', () => {
  ...
  test('builds a Program node for hello world', () => {
    ...
    expect(ast.statements[0].initializer).toEqual(new core.StringLit('Hello from Fling!'))
    expect(ast.statements[1]).toEqual(new core.ServeStmt(new core.IdRef('greeting')))
  })
  ...
  test('builds RecipeDecl nodes with parameters, return types, and bodies', () => {
    ...
    const recipe = ast.statements.find((statement) => statement instanceof core.RecipeDecl)

    expect(recipe.name).toBe('make_dough')
    expect(recipe.params).toHaveLength(2)
    expect(recipe.params[0]).toEqual(new core.Param('flour', new core.UnitType('grams')))
  ...
  test('builds control-flow, call, field access, and collection nodes', () => {
    ...
    expect(collection.statements[0].initializer).toBeInstanceOf(core.CollectionLit)
  ...
  test('parses collection type annotations', () => {
    ...
    expect(ast.statements[0].type).toEqual(new core.CollectionType(new core.PrimitiveType('count')))
  })
...
batch x in xs:
  ...
  otherwise:
    ...
    expect(ast.statements[0].body[0].alternate).toEqual([new core.ServeStmt(new core.IntLit(2))])
  })
  ...
  test('parses field access chains', () => {
    ...
    expect(initializer.object).toEqual(new core.FieldAccessExp(new core.IdRef('kitchen'), 'dough'))
  })
