...
ingredient field = xs[0].flour`,
  ...
  describe("invalid programs", () => {
    test.each([
      ...
      ["duplicate variable in same scope", "ingredient x: count = 1\ningredient x: count = 2", /already declared/],
      [
...
ingredient bad = flour + milk`,
      ...
      ["multiplying two unit types together", "ingredient bad = 1g * 2g", /cannot \* grams and grams/],
      ...
      ["equality check between different types", "ingredient bad: truth = 1 == yes", /cannot compare count and truth/],
      ["and operator on non-truth types", "ingredient bad: truth = 1 and 2", /and requires truth operands/],
      ["or operator on non-truth types", "ingredient bad: truth = yes or 2", /or requires truth operands/],
      ["not operator on non-truth type", "ingredient bad: truth = not 1", /not requires truth operand/],
      ["negation of non-numeric type", 'ingredient bad = -"no"', /negation requires numeric operand/],
      [
  ...
  step show: display(1)`,
      ...
      ["collection with mixed types", 'ingredient xs = [1, "two"]', /collection elements must have same type/],
      ["index access on non-collection", "ingredient x = 1[0]", /index access requires a collection/],
      [
...
ingredient x = xs[1.5]`,
      ...
      ["field access on non-struct type", "ingredient x = 1.flour", /cannot access field 'flour' on count/],
      [
...
ingredient x = d.water`,
      ...
      ["struct literal with wrong type name", "ingredient d = Missing { x: 1 }", /'Missing' not found/],
      [
...
ingredient y = f(1g)`,
  ...
  ingredient x: count = 1`,
      ...
      ["explicit type annotation mismatch", 'ingredient x: count = "one"', /cannot assign label to count/],
      [
